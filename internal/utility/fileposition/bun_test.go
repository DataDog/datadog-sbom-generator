...
func TestInBunLockPackages(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			// The package key contains a \uXXXX escape. It must be decoded to the
			// literal character (here, é -> é) so the returned map key
			// matches what encoding/json produces for the same lockfile;
			// otherwise callers keyed by the decoded name never find this entry.
			name: "unicode_escape_in_key",
			input: joinLines([]string{
				`{`,
				`  "packages": {`,
				`    "pkg\u00e9": ["pkg@1.0.0", "", {}, "sha512-aaa"]`,
				`  }`,
				`}`,
			}),
			expected: map[string]models.FilePosition{
		...
		{
			// The package key contains an escaped backslash (\\), which must
			// decode to a single literal backslash rather than being dropped.
			name: "backslash_escape_in_key",
			input: joinLines([]string{
				`{`,
				`  "packages": {`,
				`    "pkg\\name": ["pkg@1.0.0", "", {}, "sha512-aaa"]`,
				`  }`,
				`}`,
			}),
			expected: map[string]models.FilePosition{
		...
		{
			// Two entries share the same line. Column.Start must be each key's
			// own opening-quote column, not the first non-blank character of the
			// shared line (which would incorrectly collide both entries onto the
			// same Column.Start).
			name: "two_entries_same_line",
			input: joinLines([]string{
				`{`,
				`  "packages": {`,
				`    "a": ["a@1.0.0", "", {}, "sha512-aaa"], "b": ["b@1.0.0", "", {}, "sha512-bbb"]`,
				`  }`,
				`}`,
			}),
			expected: map[string]models.FilePosition{
				...
				"b": {
					Line:   models.Position{Start: 3, End: 3},
					Column: models.Position{Start: 45, End: 83},
				},
			},
		},
		{
