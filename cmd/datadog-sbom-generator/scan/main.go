...
Examples:
	...
	datadog-sbom-generator scan --exclude-ecosystem "npm,Go" --exclude-package "npm:lodash" .`,
		...
		Flags: []cli.Flag{
			...
			&cli.BoolFlag{
				Name:  "manifest-parsers",
				Usage: "also run parsers that extract packages from manifest files (e.g. pyproject.toml) when no lock file is present. Additive to the default parser set. Ignored when --enable-parsers is set.",
				Value: false,
			},
			&cli.StringSliceFlag{
				Name:  "exclude",
