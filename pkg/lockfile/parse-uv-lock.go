...
func (e UvLockExtractor) Extract(f DepFile) ([]PackageDetails, error) {
	...
	if len(parsedLockfile.Packages) > 0 {
		directDependencies = extractNames(parsedLockfile.Packages[len(parsedLockfile.Packages)-1].Dependencies)
	}

	devDependencies := make(map[string]struct{})
	if len(parsedLockfile.Packages) > 0 {
		var pkg = parsedLockfile.Packages[len(parsedLockfile.Packages)-1]
