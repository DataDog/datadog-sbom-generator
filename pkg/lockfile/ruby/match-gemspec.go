...
func (matcher GemspecFileMatcher) findGemspecs(node *lockfile.Node) ([]gemspecMetadata, error) {
	...
	err := node.Query(dependencyQuery, func(match *lockfile.MatchResult) error {
...
func (matcher GemspecFileMatcher) enrichPackagesWithLocation(sourceFile lockfile.DepFile, gems []gemspecMetadata, packagesByName map[string]*lockfile.PackageDetails) {
...
func setPositionInMetadata(metadata gemspecMetadata, callNode *lockfile.Node, dependencyNameNode *lockfile.Node, requirementNodes []*lockfile.Node) (gemspecMetadata, error) {
