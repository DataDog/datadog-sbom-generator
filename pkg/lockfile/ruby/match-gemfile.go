...
func (matcher GemfileMatcher) GetSourceFile(sourceFile lockfile.DepFile) (lockfile.DepFile, error) {
	...
	file, err := lockfile.OpenLocalDepFile(sourceFilePath)
...
func findGems(node *lockfile.Node) ([]gemMetadata, error) {
	...
	err := node.Query(gemQueryString, func(match *lockfile.MatchResult) error {
...
func findGroupedGems(node *lockfile.Node) ([]gemMetadata, error) {
	...
	gems := make([]gemMetadata, 0)
	err := node.Query(groupQueryString, func(match *lockfile.MatchResult) error {
...
func findGroupsInPairs(node *lockfile.Node) ([]string, error) {
	...
	var groups []string
	err := node.Query(pairQuery, func(match *lockfile.MatchResult) error {
...
func indexPackages(packages []lockfile.PackageDetails) map[string]*lockfile.PackageDetails {
...
func enrichPackagesWithLocation(sourceFile lockfile.DepFile, gems []gemMetadata, packagesByName map[string]*lockfile.PackageDetails) {
...
func setPosition(metadata gemMetadata, callNode *lockfile.Node, dependencyNameNode *lockfile.Node, requirementsNode *lockfile.Node) (gemMetadata, error) {
