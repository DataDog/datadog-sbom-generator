...
func (e MavenLockExtractor) decodeMavenFile(f lockfile.DepFile, depth int, visitedPath map[string]bool) (*MavenLockFile, error) {
	...
	if strings.HasPrefix(parentPath, "https") {
		...
		parentFile, err := lockfile.OpenLocalDepFile(parentPath)
		if err != nil {
			return nil, err
		}
...
var _ lockfile.Extractor = MavenLockExtractor{}

//nolint:gochecknoinits
func init() {
	lockfile.RegisterExtractor(models.MavenFilePath, MavenLockExtractor{})
}

func ParseMavenLock(pathToLockfile string) ([]lockfile.PackageDetails, error) {
	return lockfile.ExtractFromFile(pathToLockfile, MavenLockExtractor{})
}
