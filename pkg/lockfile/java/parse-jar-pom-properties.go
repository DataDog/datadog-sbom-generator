...
func (e JarPomPropertiesExtractor) Extract(f lockfile.DepFile, context lockfile.ScanContext) ([]lockfile.PackageDetails, error) {
	...
	if err != nil {
		return []lockfile.PackageDetails{}, fmt.Errorf("could not open %s: %w", f.Path(), err)
	}
	defer file.Close()

	stat, err := file.Stat()
	if err != nil {
		return []lockfile.PackageDetails{}, fmt.Errorf("could not stat %s: %w", f.Path(), err)
	}
...
var _ lockfile.Extractor = JarPomPropertiesExtractor{}
...
func ParseJarPomProperties(pathToLockfile string) ([]lockfile.PackageDetails, error) {
	return lockfile.ExtractFromFile(pathToLockfile, JarPomPropertiesExtractor{})
}

//nolint:gochecknoinits
func init() {
	lockfile.RegisterExtractor(models.JarFilePath, JarPomPropertiesExtractor{})
}
