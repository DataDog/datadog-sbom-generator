...
func parseDpkgPackageGroup(group []string) lockfile.PackageDetails {
	var pkg = lockfile.PackageDetails{
...
func ParseDpkgStatus(pathToLockfile string) ([]lockfile.PackageDetails, error) {
	return lockfile.ExtractFromFile(pathToLockfile, DpkgStatusExtractor{})
}
...
func (e DpkgStatusExtractor) Extract(f lockfile.DepFile, context lockfile.ScanContext) ([]lockfile.PackageDetails, error) {
	...
	packages := make([]lockfile.PackageDetails, 0, len(packageGroups))

	for _, group := range packageGroups {
		pkg := parseDpkgPackageGroup(group)

		// lockfile.PackageDetails does not contain any field that represent a "not installed" state
...
func getReleaseVersion(packages []lockfile.PackageDetails) string {
...
var _ lockfile.Extractor = DpkgStatusExtractor{}

var DpkgExtractor = DpkgStatusExtractor{}

//nolint:gochecknoinits
func init() {
	lockfile.RegisterExtractor(models.DpkgFilePath, DpkgExtractor)
}
