...
func (e MixLockExtractor) Extract(f extractor.DepFile, context extractor.ScanContext) ([]extractor.PackageDetails, error) {
	re := cachedregexp.MustCompile(`^ +"(\w+)": \{.+,$`)

	scanner := bufio.NewScanner(f)
	lineNumber := 0

	var packages []extractor.PackageDetails
...
var _ extractor.Extractor = MixLockExtractor{}

//nolint:gochecknoinits
func init() {
	extractor.RegisterExtractor(models.HexFilePath, MixLockExtractor{})
}

func ParseMixLock(pathToLockfile string) ([]extractor.PackageDetails, error) {
	return extractor.ExtractFromFile(pathToLockfile, MixLockExtractor{})
}
