...
func parseConanV1Lock(sourceFile ConanLockFile, positions map[string]*models.FilePosition, filePath string) []extractor.PackageDetails {
	var reference ConanReference
	packages := make([]extractor.PackageDetails, 0, len(sourceFile.GraphLock.Nodes))
...
func parseConanRequires(packages *[]extractor.PackageDetails, requires []string, group string, lines []string, filePath string) {
...
func parseConanV2Lock(sourceFile ConanLockFile, lines []string, filePath string) []extractor.PackageDetails {
	packages := make(
...
func parseConanLock(lockfile ConanLockFile, lines []string, filePath string) []extractor.PackageDetails {
...
func (e ConanLockExtractor) Extract(f extractor.DepFile, context extractor.ScanContext) ([]extractor.PackageDetails, error) {
...
var _ extractor.Extractor = ConanLockExtractor{}

//nolint:gochecknoinits
func init() {
	extractor.RegisterExtractor(models.ConanFilePath, ConanLockExtractor{})
}

func ParseConanLock(pathToLockfile string) ([]extractor.PackageDetails, error) {
	return extractor.ExtractFromFile(pathToLockfile, ConanLockExtractor{})
}
