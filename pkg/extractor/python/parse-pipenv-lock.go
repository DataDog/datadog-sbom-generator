...
func (e PipenvLockExtractor) Extract(f extractor.DepFile, context extractor.ScanContext) ([]extractor.PackageDetails, error) {
	...
	details := make(map[string]extractor.PackageDetails)
...
var PipenvExtractor = PipenvLockExtractor{
	extractor.WithMatcher{Matchers: []extractor.Matcher{&PipfileMatcher{}}},
}

func ParsePipenvLock(pathToLockfile string) ([]extractor.PackageDetails, error) {
	return extractor.ExtractFromFile(pathToLockfile, PipenvExtractor)
}

//nolint:gochecknoinits
func init() {
	extractor.RegisterExtractor(models.PipfileFilePath, PipenvExtractor)
}
