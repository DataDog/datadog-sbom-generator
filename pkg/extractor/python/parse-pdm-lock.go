...
func (p PdmLockExtractor) Extract(f extractor.DepFile, context extractor.ScanContext) ([]extractor.PackageDetails, error) {
	...
	for _, pkg := range parsedLockFile.Packages {
		details := extractor.PackageDetails{
			Name:           pkg.Name,
			Version:        pkg.Version,
...
var _ extractor.Extractor = PdmLockExtractor{}

//nolint:gochecknoinits
func init() {
	extractor.RegisterExtractor(models.PdmFilePath, PdmLockExtractor{})
}

func ParsePdmLock(pathToLockfile string) ([]extractor.PackageDetails, error) {
	return extractor.ExtractFromFile(pathToLockfile, PdmLockExtractor{})
}
