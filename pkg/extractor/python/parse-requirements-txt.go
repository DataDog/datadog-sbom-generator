...
func parseLine(path string, line string, lineNumber int, lineOffset int, columnStart int, columnEnd int) extractor.PackageDetails {
...
func (e RequirementsTxtExtractor) Extract(f extractor.DepFile, context extractor.ScanContext) ([]extractor.PackageDetails, error) {
	return parseRequirementsTxt(f, map[string]struct{}{})
}

func parseRequirementsTxt(f extractor.DepFile, requiredAlready map[string]struct{}) ([]extractor.PackageDetails, error) {
	...
	lastPkg := extractor.PackageDetails{}
	lastPkgKey := ""
	...
	for scanner.Scan() {
		...
		if autoGenerated {
			detail.LocationRole = models.LocationRoleLockfile
		}
...
func init() {
	extractor.RegisterExtractor(models.RequirementsFilePath, RequirementsExtractor)
}

func ParseRequirementsTxt(pathToLockfile string) ([]extractor.PackageDetails, error) {
	return extractor.ExtractFromFile(pathToLockfile, RequirementsExtractor)
}
