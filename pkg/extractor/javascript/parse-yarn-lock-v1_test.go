...
func TestParseYarnLock_v1_OnePackage_MatcherFailed(t *testing.T) {
	...
	javascript.YarnExtractor.Matchers = []extractor.Matcher{testutil.FailingMatcher{Error: matcherError}}
...
func TestParseYarnLock_v1_Files(t *testing.T) {
	...
	expected := []extractor.PackageDetails{
		{
			Name:           "etag",
			Version:        "1.8.1",
			PackageManager: models.Yarn,
...
func TestParseYarnLock_v1_WorkspacesSameLibSameVersion(t *testing.T) {
	...
	// sourceFile, err := extractor.OpenLocalDepFile("/private/tmp/sbom-test-repos/sca-testing-electron/spec/package.json")
	if err != nil {
		t.Errorf("Got unexpected error: %v", err)
	}
	...
	testutil.ExpectPackages(t, packages, []extractor.PackageDetails{
		{
			...
			Dependencies: []*extractor.PackageDetails{
				{
					Name:           "colors",
					Version:        "1.4.0",
