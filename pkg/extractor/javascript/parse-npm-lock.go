...
func mergeNpmDepsGroups(a, b extractor.PackageDetails) []string {
...
func (pdm npmPackageDetailsMap) add(key string, details extractor.PackageDetails) {
	existing, ok := pdm[key]
...
func parseNpmLock(lockfile NpmLockfile, lines []string) map[string]extractor.PackageDetails {
...
func ParseNpmLock(pathToLockfile string) ([]extractor.PackageDetails, error) {
	return extractor.ExtractFromFile(pathToLockfile, NpmExtractor)
}

//nolint:gochecknoinits
func init() {
	extractor.RegisterExtractor(models.NpmFilePath, NpmExtractor)
}
