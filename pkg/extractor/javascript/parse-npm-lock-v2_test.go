...
func TestParseNpmLock_v2_OnePackage_MatcherFailed(t *testing.T) {
	...
	javascript.NpmExtractor.Matchers = []extractor.Matcher{testutil.FailingMatcher{Error: matcherError}}
...
func TestParseNpmLock_v2_SamePackageDifferentGroups(t *testing.T) {
	...
	packages, err := javascript.ParseNpmLock("../fixtures/npm/same-package-different-groups.v2.json")
	if err != nil {
		t.Errorf("Got unexpected error: %v", err)
	}
...
func TestParseNpmLock_v2_Workspaces(t *testing.T) {
	...
	sourceFile, err := extractor.OpenLocalDepFile("../fixtures/package-json/workspaces/package.json")
	if err != nil {
		t.Errorf("Got unexpected error: %v", err)
	}
...
func TestParseNpmLock_v2_WorkspacesSameLibSameVersion(t *testing.T) {
	...
	sourceFile, err := extractor.OpenLocalDepFile("../fixtures/package-json/workspace-same-lib-and-version/package.json")
	if err != nil {
		t.Errorf("Got unexpected error: %v", err)
	}
	err = packageJSONMatcher.Match(sourceFile, packages, testutil.GetTestContext())
...
func TestParseNpmLock_v2_WorkspacesSameLibDifferentVersion(t *testing.T) {
	...
	sourceFile, err := extractor.OpenLocalDepFile("../fixtures/package-json/workspace-same-lib-different-version/package.json")
	if err != nil {
		t.Errorf("Got unexpected error: %v", err)
	}
	err = packageJSONMatcher.Match(sourceFile, packages, testutil.GetTestContext())
...
func TestParseNpmLock_v2_WorkspacesComplex(t *testing.T) {
	t.Parallel()
	...
	path := filepath.FromSlash(filepath.Join(dir, "../fixtures/package-json/workspace-complex/package-lock.json"))
	packages, err := javascript.ParseNpmLock(path)
	if err != nil {
		t.Errorf("Got unexpected error: %v", err)
	}

	sourceFile, err := extractor.OpenLocalDepFile("../fixtures/package-json/workspace-complex/package.json")
	if err != nil {
		t.Errorf("Got unexpected error: %v", err)
	}
	err = packageJSONMatcher.Match(sourceFile, packages, testutil.GetTestContext())
