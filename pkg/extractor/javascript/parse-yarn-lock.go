...
*/
func buildDependencyTree(rootPkgName, rootPkgTargetVersion, rootPkgRegistry string, dependencies map[string]YarnPackage, packagesIndex map[string]*extractor.PackageDetails) []*extractor.PackageDetails {
	results := make([]*extractor.PackageDetails, 0)
...
func parseYarnPackage(dependency YarnPackage, filePath string) extractor.PackageDetails {
...
func indexByNameAndVersions(packages []extractor.PackageDetails) map[string]*extractor.PackageDetails {
...
func (e YarnLockExtractor) Extract(f extractor.DepFile, context extractor.ScanContext) ([]extractor.PackageDetails, error) {
	...
	if err != nil {
		return []extractor.PackageDetails{}, fmt.Errorf("error reading yarn.lock: %w", err)
	}
...
func createPackageDetails(allResolvedPackages []YarnPackage, dependencyWorkspaces map[string][]string, filePath string) []extractor.PackageDetails {
	packages := make([]extractor.PackageDetails, 0, len(allResolvedPackages))

	// Create extractor.PackageDetails for regular packages, with workspace information where applicable
	for _, yarnPackage := range allResolvedPackages {
		...
		if workspacePaths, exists := dependencyWorkspaces[depKey]; exists {
			// Create separate extractor.PackageDetails for each workspace that declares this dependency with this target version
...
func ParseYarnLock(pathToLockfile string) ([]extractor.PackageDetails, error) {
	return extractor.ExtractFromFile(pathToLockfile, YarnExtractor)
}

//nolint:gochecknoinits
func init() {
	extractor.RegisterExtractor(models.YarnFilePath, YarnExtractor)
}
