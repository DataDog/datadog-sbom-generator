...
func hasLockfileInAncestors(f extractor.DepFile, rootDir string, lockfiles []string) bool {
...
func (e PackageJSONExtractor) Extract(f extractor.DepFile, context extractor.ScanContext) ([]extractor.PackageDetails, error) {
	...
	for _, section := range sections {
		...
		for _, name := range names {
			...
			packages[dedupKey] = extractor.PackageDetails{
				Name:                         resolvedName,
				Version:                      version,
	...
	result := make([]extractor.PackageDetails, 0, len(packages))
...
func ParsePackageJSON(path string) ([]extractor.PackageDetails, error) {
	return extractor.ExtractFromFile(path, packageJSONExtractor)
}

//nolint:gochecknoinits
func init() {
	extractor.RegisterExtractor(models.PackageJSONFilePath, packageJSONExtractor)
}
