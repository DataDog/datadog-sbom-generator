package dart_test

import (
	"io/fs"
	"path/filepath"
	"testing"

	"github.com/stretchr/testify/assert"

	"github.com/DataDog/datadog-sbom-generator/pkg/extractor"
	"github.com/DataDog/datadog-sbom-generator/pkg/extractor/dart"
	"github.com/DataDog/datadog-sbom-generator/pkg/extractor/internal/testutil"
	"github.com/DataDog/datadog-sbom-generator/pkg/models"
)

func TestPubspecLockExtractor_ShouldExtract(t *testing.T) {
	t.Parallel()

	tests := []struct {
		name string
		path string
		want bool
	}{
		{
			name: "",
			path: "",
			want: false,
		},
		{
			name: "",
			path: "pubspec.lock",
			want: true,
		},
		{
			name: "",
			path: "path/to/my/pubspec.lock",
			want: true,
		},
		{
			name: "",
			path: "path/to/my/pubspec.lock/file",
			want: false,
		},
		{
			name: "",
			path: "path/to/my/pubspec.lock.file",
			want: false,
		},
		{
			name: "",
			path: "path.to.my.pubspec.lock",
			want: false,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			e := dart.PubspecLockExtractor{}
			got := e.ShouldExtract(tt.path)
			if got != tt.want {
				t.Errorf("Extract() got = %v, want %v", got, tt.want)
			}
		})
	}
}

func TestParsePubspecLock_FileDoesNotExist(t *testing.T) {
	t.Parallel()

	packages, err := dart.ParsePubspecLock("../fixtures/pub/does-not-exist")

	testutil.ExpectErrIs(t, err, fs.ErrNotExist)
	testutil.ExpectPackages(t, packages, []extractor.PackageDetails{})
}

func TestParsePubspecLock_InvalidYaml(t *testing.T) {
	t.Parallel()

	packages, err := dart.ParsePubspecLock("../fixtures/pub/not-yaml.txt")

	testutil.ExpectErrContaining(t, err, "could not extract from")
	testutil.ExpectPackages(t, packages, []extractor.PackageDetails{})
}

func TestParsePubspecLock_Empty(t *testing.T) {
	t.Parallel()

	packages, err := dart.ParsePubspecLock("../fixtures/pub/empty.lock")

	if err != nil {
		t.Errorf("Got unexpected error: %v", err)
	}

	testutil.ExpectPackages(t, packages, []extractor.PackageDetails{})
}

func TestParsePubspecLock_NoPackages(t *testing.T) {
	t.Parallel()

	packages, err := dart.ParsePubspecLock("../fixtures/pub/no-packages.lock")

	if err != nil {
		t.Errorf("Got unexpected error: %v", err)
	}

	testutil.ExpectPackages(t, packages, []extractor.PackageDetails{})
}

func TestParsePubspecLock_OnePackage(t *testing.T) {
	t.Parallel()

	packages, err := dart.ParsePubspecLock("../fixtures/pub/one-package.lock")

	if err != nil {
		t.Errorf("Got unexpected error: %v", err)
	}

	testutil.ExpectPackagesWithoutLocations(t, packages, []extractor.PackageDetails{
		{
			Name:           "back_button_interceptor",
			Version:        "6.0.1",
			PackageManager: models.Pub,
			Ecosystem:      models.EcosystemPub,
		},
	})
}

func TestParsePubspecLock_OnePackageDev(t *testing.T) {
	t.Parallel()

	packages, err := dart.ParsePubspecLock("../fixtures/pub/one-package-dev.lock")

	if err != nil {
		t.Errorf("Got unexpected error: %v", err)
	}

	testutil.ExpectPackagesWithoutLocations(t, packages, []extractor.PackageDetails{
		{
			Name:           "build_runner",
			Version:        "2.2.1",
			PackageManager: models.Pub,
			Ecosystem:      models.EcosystemPub,
			DepGroups:      []string{"dev"},
		},
	})
}

func TestParsePubspecLock_TwoPackages(t *testing.T) {
	t.Parallel()

	packages, err := dart.ParsePubspecLock("../fixtures/pub/two-packages.lock")

	if err != nil {
		t.Errorf("Got unexpected error: %v", err)
	}

	testutil.ExpectPackagesWithoutLocations(t, packages, []extractor.PackageDetails{
		{
			Name:           "shelf",
			Version:        "1.3.2",
			PackageManager: models.Pub,
			Ecosystem:      models.EcosystemPub,
		},
		{
			Name:           "shelf_web_socket",
			Version:        "1.0.2",
			PackageManager: models.Pub,
			Ecosystem:      models.EcosystemPub,
		},
	})
}

func TestParsePubspecLock_MixedPackages(t *testing.T) {
	t.Parallel()

	packages, err := dart.ParsePubspecLock("../fixtures/pub/mixed-packages.lock")

	if err != nil {
		t.Errorf("Got unexpected error: %v", err)
	}

	testutil.ExpectPackagesWithoutLocations(t, packages, []extractor.PackageDetails{
		{
			Name:           "back_button_interceptor",
			Version:        "6.0.1",
			PackageManager: models.Pub,
			Ecosystem:      models.EcosystemPub,
		},
		{
			Name:           "build_runner",
			Version:        "2.2.1",
			PackageManager: models.Pub,
			Ecosystem:      models.EcosystemPub,
			DepGroups:      []string{"dev"},
		},
		{
			Name:           "shelf",
			Version:        "1.3.2",
			PackageManager: models.Pub,
			Ecosystem:      models.EcosystemPub,
		},
		{
			Name:           "shelf_web_socket",
			Version:        "1.0.2",
			PackageManager: models.Pub,
			Ecosystem:      models.EcosystemPub,
		},
	})
}

func TestParsePubspecLock_PackageWithGitSource(t *testing.T) {
	t.Parallel()

	packages, err := dart.ParsePubspecLock("../fixtures/pub/source-git.lock")

	if err != nil {
		t.Errorf("Got unexpected error: %v", err)
	}

	testutil.ExpectPackagesWithoutLocations(t, packages, []extractor.PackageDetails{
		{
			Name:           "flutter_rust_bridge",
			Version:        "1.32.0",
			PackageManager: models.Pub,
			Ecosystem:      models.EcosystemPub,
			Commit:         "e5adce55eea0b74d3680e66a2c5252edf17b07e1",
		},
		{
			Name:           "screen_retriever",
			Version:        "0.1.2",
			PackageManager: models.Pub,
			Ecosystem:      models.EcosystemPub,
			Commit:         "406b9b038b2c1d779f1e7bf609c8c248be247372",
		},
		{
			Name:           "tray_manager",
			Version:        "0.1.8",
			PackageManager: models.Pub,
			Ecosystem:      models.EcosystemPub,
			Commit:         "3aa37c86e47ea748e7b5507cbe59f2c54ebdb23a",
		},
		{
			Name:           "window_manager",
			Version:        "0.2.7",
			PackageManager: models.Pub,
			Ecosystem:      models.EcosystemPub,
			Commit:         "88487257cbafc501599ab4f82ec343b46acec020",
		},
		{
			Name:           "toggle_switch",
			Version:        "1.4.0",
			PackageManager: models.Pub,
			Ecosystem:      models.EcosystemPub,
			Commit:         "",
		},
	})
}

func TestParsePubspecLock_PackageWithSdkSource(t *testing.T) {
	t.Parallel()

	packages, err := dart.ParsePubspecLock("../fixtures/pub/source-sdk.lock")

	if err != nil {
		t.Errorf("Got unexpected error: %v", err)
	}

	testutil.ExpectPackagesWithoutLocations(t, packages, []extractor.PackageDetails{
		{
			Name:           "flutter_web_plugins",
			Version:        "0.0.0",
			PackageManager: models.Pub,
			Ecosystem:      models.EcosystemPub,
			Commit:         "",
		},
	})
}

func TestParsePubspecLock_PackageWithPathSource(t *testing.T) {
	t.Parallel()

	packages, err := dart.ParsePubspecLock("../fixtures/pub/source-path.lock")

	if err != nil {
		t.Errorf("Got unexpected error: %v", err)
	}

	testutil.ExpectPackagesWithoutLocations(t, packages, []extractor.PackageDetails{
		{
			Name:           "maa_core",
			Version:        "0.0.1",
			PackageManager: models.Pub,
			Ecosystem:      models.EcosystemPub,
			Commit:         "",
		},
	})
}

func TestParsePubspecLock_TwoPackages_BlockLocation(t *testing.T) {
	t.Parallel()

	packages, err := dart.ParsePubspecLock("../fixtures/pub/two-packages.lock")

	if err != nil {
		t.Errorf("Got unexpected error: %v", err)
	}

	packagesByName := make(map[string]extractor.PackageDetails)
	for _, pkg := range packages {
		packagesByName[pkg.Name] = pkg
	}

	absoluteLockfilePath, err := filepath.Abs("../fixtures/pub/two-packages.lock")
	if err != nil {
		t.Fatalf("Could not get absolute path: %v", err)
	}

	// shelf: lines 4-10
	shelfPkg := packagesByName["shelf"]
	assert.Equal(t, absoluteLockfilePath, shelfPkg.BlockLocation.Filename)
	assert.Equal(t, 4, shelfPkg.BlockLocation.Line.Start)
	assert.Equal(t, 10, shelfPkg.BlockLocation.Line.End)

	// shelf_web_socket: lines 11-17
	swsPkg := packagesByName["shelf_web_socket"]
	assert.Equal(t, absoluteLockfilePath, swsPkg.BlockLocation.Filename)
	assert.Equal(t, 11, swsPkg.BlockLocation.Line.Start)
	assert.Equal(t, 17, swsPkg.BlockLocation.Line.End)
}
