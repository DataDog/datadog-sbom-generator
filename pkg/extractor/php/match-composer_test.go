...
func TestComposerMatcher_GetSourceFile_FileDoesNotExist(t *testing.T) {
	...
	lockFile, err := extractor.OpenLocalDepFile("../fixtures/composer/no-json/composer.lock")
	if err != nil {
...
func TestComposerMatcher_GetSourceFile(t *testing.T) {
	...
	lockFile, err := extractor.OpenLocalDepFile(basePath + "composer.lock")
	if err != nil {
		t.Errorf("Got unexpected error: %v", err)
	}
...
func TestComposerMatcher_Match_OnePackage(t *testing.T) {
	...
	sourceFile, err := extractor.OpenLocalDepFile("../fixtures/composer/one-package/composer.json")
	if err != nil {
		t.Errorf("Got unexpected error: %v", err)
	}
