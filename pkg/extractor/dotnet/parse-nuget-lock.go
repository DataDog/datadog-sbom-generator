...
func parseNuGetLock(
	...
	lines []string,
	filePath string,
) ([]extractor.PackageDetails, error) {
	details := map[string]extractor.PackageDetails{}
...
func (e NuGetLockExtractor) Extract(f extractor.DepFile, context extractor.ScanContext) ([]extractor.PackageDetails, error) {
	...
	if parsedLockfile.Version != 1 && parsedLockfile.Version != 2 {
		return []extractor.PackageDetails{}, fmt.Errorf("could not extract: unsupported lock file version %d", parsedLockfile.Version)
	}
...
var NuGetExtractor = NuGetLockExtractor{
	extractor.WithMatcher{Matchers: []extractor.Matcher{&NugetCsprojMatcher{}}},
}

func ParseNuGetLock(pathToLockfile string) ([]extractor.PackageDetails, error) {
	return extractor.ExtractFromFile(pathToLockfile, NuGetExtractor)
}

//nolint:gochecknoinits
func init() {
	extractor.RegisterExtractor(models.NugetLockFilePath, NuGetExtractor)
}
