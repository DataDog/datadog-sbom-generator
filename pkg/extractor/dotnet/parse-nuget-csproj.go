...
func (e NuGetCsprojExtractor) Extract(f extractor.DepFile, context extractor.ScanContext) ([]extractor.PackageDetails, error) {
	...
	packageMap := make(map[packageKey]*extractor.PackageDetails)
		...
		for name, pkgRef := range packagesByName {
			...
			for _, version := range versions {
				...
				if existing, exists := packageMap[key]; !exists {
					packageMap[key] = &extractor.PackageDetails{
						Name:            name,
						Version:         version,
	...
	details := make([]extractor.PackageDetails, 0, len(packageMap))
...
var _ extractor.Extractor = NuGetCsprojExtractor{}
...
func init() {
	extractor.RegisterExtractor(models.NuGetCsProjFilePath, NuGetCsprojExtractor{})
}

func ParseNuGetCsproj(pathToCsproj string) ([]extractor.PackageDetails, error) {
	return extractor.ExtractFromFile(pathToCsproj, NuGetCsprojExtractor{})
}

func ParseNuGetCsprojWithContext(pathToCsproj string, context extractor.ScanContext) ([]extractor.PackageDetails, error) {
	return extractor.ExtractFromFileWithContext(pathToCsproj, NuGetCsprojExtractor{}, context)
}
