...
func parseDpkgPackageGroup(group []string) extractor.PackageDetails {
	var pkg = extractor.PackageDetails{
...
func ParseDpkgStatus(pathToLockfile string) ([]extractor.PackageDetails, error) {
	return extractor.ExtractFromFile(pathToLockfile, DpkgStatusExtractor{})
}

type DpkgStatusExtractor struct{}

func (e DpkgStatusExtractor) ShouldExtract(path string) bool {
	return path == models.DpkgFilePath.String()
}
...
func (e DpkgStatusExtractor) Extract(f extractor.DepFile, context extractor.ScanContext) ([]extractor.PackageDetails, error) {
	scanner := bufio.NewScanner(f)
	packageGroups := groupDpkgPackageLines(scanner)

	packages := make([]extractor.PackageDetails, 0, len(packageGroups))

	for _, group := range packageGroups {
		pkg := parseDpkgPackageGroup(group)

		// extractor.PackageDetails does not contain any field that represent a "not installed" state
...
func getReleaseVersion(packages []extractor.PackageDetails) string {
...
var _ extractor.Extractor = DpkgStatusExtractor{}

var DpkgExtractor = DpkgStatusExtractor{}

//nolint:gochecknoinits
func init() {
	extractor.RegisterExtractor(models.DpkgFilePath, DpkgExtractor)
}
