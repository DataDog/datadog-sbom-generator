...
func (m CargoTomlMatcher) GetSourceFile(depFile extractor.DepFile) (extractor.DepFile, error) {
	return depFile.Open("Cargo.toml")
}
...
func processDependencySection(deps map[string]interface{}, packages []extractor.PackageDetails, lines []string, filePath string, depGroup string) {
	if deps == nil {
		return
	}
...
func findPackagePositions(pkg *extractor.PackageDetails, depName string, versionReq string, lines []string, filePath string) bool {
...
var _ extractor.Matcher = CargoTomlMatcher{}
