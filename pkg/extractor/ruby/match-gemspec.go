...
func (matcher GemspecFileMatcher) Match(sourceFile extractor.DepFile, packages []extractor.PackageDetails, context extractor.ScanContext) error {
...
func (matcher GemspecFileMatcher) findGemspecs(node *extractor.Node) ([]gemspecMetadata, error) {
	...
	err := node.Query(dependencyQuery, func(match *extractor.MatchResult) error {
