...
func Test_ParseFile(t *testing.T) {
	...
	sourcefile, err := extractor.OpenLocalDepFile("fixtures/bundler/groups/Gemfile")
	if err != nil {
		t.Fatalf("Got unexpected error: %v", err)
	}

	res, err := extractor.ParseFile(sourcefile, extractor.Ruby)
	if err != nil {
		t.Fatalf("Got unexpected error: %v", err)
	}
	defer res.Close()
	...
	)`, func(match *extractor.MatchResult) error {
		gemsCount++
...
func Test_ParseFile_BadQuery(t *testing.T) {
	...
	sourcefile, err := extractor.OpenLocalDepFile("fixtures/bundler/one-package/Gemfile")
	if err != nil {
		t.Fatalf("Got unexpected error: %v", err)
	}
	...
	err = res.Node.Query(`((call)`, func(match *extractor.MatchResult) error {
...
func Test_ParseFile_Error(t *testing.T) {
	...
	sourcefile, err := extractor.OpenLocalDepFile("fixtures/bundler/groups/Gemfile.lock")
	if err != nil {
		t.Fatalf("Got unexpected error: %v", err)
	}

	_, err = extractor.ParseFile(sourcefile, extractor.Ruby)
